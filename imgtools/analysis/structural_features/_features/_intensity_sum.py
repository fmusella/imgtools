...
def run(feat_arr: np.ndarray, cell_data: dict, index: Index) -> tuple:
    ...
    If there are two or more spots corresponding to the same domain in the trace, their intensities are summed.
    ...
    for (i_domain, i_trace), vals in feat_per_domain.items():
        feat_arr[i_domain, i_trace] = np.sum(vals)  # TODO: so similar to _intensity. I should change the code so that I can separate the name of the package and the name of the feature to store, so that I can easily reuse the code for different packages
